...
class PokemonEntry:
    ...
    def species(self) -> str:
        ...
        for match in matches:
            if match not in {"M", "F"}:
                return match.removeprefix("SPECIES_")
...
def parse_pokemon(chunk: str) -> PokemonEntry | None:
    ...
    title_gender = ""
    if re.search(r"\(M\)", mon.title):
        title_gender = "Male"
    elif re.search(r"\(F\)", mon.title):
        title_gender = "Female"
    ...
    for line in lines[1:]:
        ...
        if stripped.startswith("- "):
            ...
            mon.fields["Level"] = stripped[6:].strip()
        elif stripped.endswith(" Nature"):
            mon.fields["Nature"] = stripped.split(" ")[0].strip()
        else:
            mon.extra_lines.append(stripped)
            
    if title_gender and "Gender" not in mon.fields:
        mon.fields["Gender"] = title_gender
...
def normalize_name(value: str) -> str:
    value = value.strip().removeprefix("SPECIES_").removeprefix("TRAINER_PIC_FRONT_").removeprefix("TRAINER_PIC_")
...
def load_trainer_pic_paths() -> tuple[dict[str, Path], list[str]]:
    ...
    if front_pics_dir.exists():
        for png in front_pics_dir.glob("*.png"):
            name = png.stem
            label = name.replace("_", " ").title()
            if label not in choices:
                choices.append(label)
            if normalize_name(label) not in paths:
                paths[normalize_name(label)] = png
            if snake_name(label) not in paths:
                paths[snake_name(label)] = png
...
def load_ball_choices(item_choices: list[str]) -> list[str]:
    return [item.removesuffix(" Ball").upper() for item in item_choices if item.endswith(" Ball")]
...
def pokemon_preview_path(species: str) -> Path | None:
    ...
    for name in ("anim_front.png", "front.png", "icon.png"):
        ...
        if path.exists():
            return path
    snaked = re.sub(r"[^a-z0-9_]", "", snake_name(species))
    folders = [POKEMON_DIR / snaked]
    ...
    if "_" in snaked:
        ...
        for i in range(1, len(parts)):
            base = "_".join(parts[:i])
            form = "_".join(parts[i:])
            folders.append(POKEMON_DIR / base / form)
            
            if form == "alola":
                folders.append(POKEMON_DIR / base / "alolan")
            elif form == "galar":
                folders.append(POKEMON_DIR / base / "galarian")
            elif form == "hisui":
                folders.append(POKEMON_DIR / base / "hisuian")
            elif form == "paldea":
                folders.append(POKEMON_DIR / base / "paldean")
...
def pokemon_icon_path(species: str) -> Path | None:
    folder = POKEMON_DIR / snake_name(species)
    for name in ("icon.png", "anim_front.png", "front.png"):
        path = folder / name
        if path.exists():
            return path
    snaked = re.sub(r"[^a-z0-9_]", "", snake_name(species))
...
class PartyTab(ttk.Frame):
    def __init__(self, parent: ttk.Notebook, title: str, path: Path, trainer_pics: dict[str, Path], pic_choices: list[str], species_choices: list[str], move_choices: list[str], item_choices: list[str], ability_choices: list[str], species_ability_choices: dict[str, list[str]], type_choices: list[str], nature_choices: list[str], ball_choices: list[str]) -> None:
        ...
        self.mon_current_frames: list[tk.PhotoImage] = []
        self.mon_images: list[tk.PhotoImage] = []
        self.animated_labels: list[tuple[tk.Widget, list[tk.PhotoImage]]] = []
    ...
    def animate_icons(self):
        if not self.winfo_exists():
            return
        self.animation_step = 1 - self.animation_step
        for label, frames in self.animated_labels:
            try:
                if frames and len(frames) > self.animation_step:
                    label.configure(image=frames[self.animation_step])
                elif frames:
                    label.configure(image=frames[0])
            except tk.TclError:
                pass
        self.after(500, self.animate_icons)
    ...
    def build_ui(self, title: str) -> None:
        ...
        self.diff_notebook = ttk.Notebook(left)
        self.diff_notebook.pack(fill=tk.BOTH, expand=True)
        self.diff_notebook.bind("<<NotebookTabChanged>>", lambda e: self.refresh_trainers())
        self.trainer_lists: dict[str, tk.Listbox] = {}
        self.diff_frames: dict[str, ttk.Frame] = {}
        ...
        ttk.Button(trainer_controls, text="Generate Poryscript", command=self.generate_poryscript).pack(side=tk.LEFT)
        ...
        for row, key in enumerate(["Battle Type", "Mugshot", "Starting Status", "Multi Party", "Difficulty"], start=3):
        ...
        self.party_list = tk.Listbox(party_left, exportselection=False, height=12, width=40)
        ...
        move_btn_frame = ttk.Frame(party_left)
        move_btn_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(4, 0))
        move_btn_frame.columnconfigure(0, weight=1)
        move_btn_frame.columnconfigure(1, weight=1)
        ttk.Button(move_btn_frame, text="Move Up", command=self.move_pokemon_up).grid(row=0, column=0, sticky="ew", padx=(0, 4))
        ttk.Button(move_btn_frame, text="Move Down", command=self.move_pokemon_down).grid(row=0, column=1, sticky="ew")
        ...
        mon_right.columnconfigure(2, weight=1)
        ...
        left_fields = ["Ability", "Level", "Ball", "Happiness", "Gender", "Nature", "Shiny"]
        for index, key in enumerate(left_fields):
            ...
            if key == "Ability":
                ...
                ttk.Combobox(mon_right, textvariable=var, values=["Male", "Female", "Random", "Genderless"], width=16).grid(row=row, column=1, sticky="ew", padx=6, pady=3)
            elif key == "Shiny":
        ...
        for key, row in (("Dynamax Level", 2), ("Gigantamax", 3), ("Tera Type", 4)):
            ...
            var.trace_add("write", self.on_mon_form_change)
            self.mon_vars[key] = var
            label_text = "Dynamax:" if key == "Dynamax Level" else f"{key}:"
            ttk.Label(mon_right, text=label_text).grid(row=row, column=2, sticky="w", padx=(12, 4), pady=3)
            if key in ("Gigantamax", "Dynamax Level"):
        ...
        ttk.Label(stats_frame, text="IVs:").grid(row=1, column=0, sticky="e", padx=(0, 3))
        ttk.Label(stats_frame, text="EVs:").grid(row=2, column=0, sticky="e", padx=(0, 3))
        for col, stat in enumerate(STAT_FIELDS):
            ttk.Label(stats_frame, text=stat).grid(row=0, column=col + 1, sticky="s")
            iv_var = tk.StringVar()
            ev_var = tk.StringVar()
            ...
            ttk.Entry(stats_frame, textvariable=iv_var, width=2).grid(row=1, column=col + 1, padx=1)
            ttk.Entry(stats_frame, textvariable=ev_var, width=3).grid(row=2, column=col + 1, padx=1, pady=(4, 0))
    ...
    def refresh_diff_tabs(self) -> None:
        ...
        current_diff = ""
        if hasattr(self, 'diff_notebook') and self.diff_notebook.tabs():
            current_idx = self.diff_notebook.index("current")
            for d, frame in self.diff_frames.items():
                if str(frame) == self.diff_notebook.tabs()[current_idx]:
                    current_diff = d
                    break

        for child in self.diff_notebook.winfo_children():
            child.destroy()

        self.trainer_lists = {}
        self.diff_frames = {}
        for diff in diffs:
            frame = ttk.Frame(self.diff_notebook)
            self.diff_frames[diff] = frame
            tab_text = diff if diff else "Normal"
            self.diff_notebook.add(frame, text=tab_text)

            lst = tk.Listbox(frame, exportselection=False, width=24)
            lst = tk.Listbox(frame, exportselection=False, width=40)
            lst.pack(fill=tk.BOTH, expand=True)
            lst.bind("<<ListboxSelect>>", self.on_trainer_select)
            self.trainer_lists[diff] = lst

            if diff == current_diff:
                self.diff_notebook.select(frame)
    ...
    def refresh_trainers(self) -> None:
        ...
        self.visible_trainers = [
            ...
            if trainer.fields.get("Difficulty", "").strip() == current_diff and (
                query in trainer.trainer_id.lower()
                or query in trainer.fields.get("Name", "").lower()
                or query in trainer.fields.get("Class", "").lower()
            )
        ]
        lst = self.trainer_lists.get(current_diff)
        if not lst:
            return
            
        lst.delete(0, tk.END)
        for index, trainer in enumerate(self.visible_trainers):
            lst.insert(tk.END, f"{index:03X}  {trainer.label}")
    ...
    def clear_mon_form(self) -> None:
        ...
        if "Dynamax Level" in self.mon_vars:
            self.mon_vars["Dynamax Level"].set("No")
    ...
    def on_mon_select(self, _event: object | None = None) -> None:
        ...
        title_species = mon.title.split("@", 1)[0].strip()
        title_species = re.sub(r"\s*\([MF]\)\s*", "", title_species).strip()
        self.mon_title.set(title_species)
        ...
        for key in MON_SIMPLE_FIELDS:
            ...
            if key in {"Shiny", "Gigantamax", "Dynamax Level"} and not value:
                value = "No"
            elif key == "Dynamax Level" and value and value != "No":
                value = "Yes"
    ...
    def save_current(self) -> None:
        ...
        current_diff = self.current.fields.get("Difficulty", "").strip()
        if current_diff in self.diff_frames:
            self.diff_notebook.select(self.diff_frames[current_diff])
            self.refresh_trainers()
    ...
    def generate_poryscript(self) -> None:
        ...
        trainer_id = self.current.trainer_id
        is_double = self.current.fields.get("Double Battle", "No").strip().lower() == "yes"
        
        dialog = tk.Toplevel(self)
        dialog.title("Generate Poryscript")
        ...
        ttk.Label(top_frame, text="Battle Type:").pack(side=tk.LEFT)
        battle_type_var = tk.StringVar(value="Double" if is_double else "Single")
        ttk.Combobox(top_frame, textvariable=battle_type_var, values=["Single", "Double", "Multi (1v2)", "Multi (2v2)"], state="readonly").pack(side=tk.LEFT, padx=6)
        
        text_widget = tk.Text(dialog, wrap=tk.NONE)
        text_widget.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=8, pady=8)
        
        def update_script(*_args: object) -> None:
            b_type = battle_type_var.get()
            script_name = trainer_id.replace("TRAINER_", "").title().replace("_", "")
            lines = []
            lines.append(f"script EventScript_{script_name} {{")
            if b_type == "Single":
                lines.append(f"    trainerbattle_single({trainer_id}, {script_name}_Text_Intro, {script_name}_Text_Defeat)")
                lines.append(f"    msgbox({script_name}_Text_PostBattle, MSGBOX_AUTOCLOSE)")
                lines.append("    end")
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_Intro {{")
                lines.append('    "Let\'s battle!"')
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_Defeat {{")
                lines.append('    "I lost..."')
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_PostBattle {{")
                lines.append('    "Good battle."')
                lines.append("}")
            elif b_type == "Double":
                lines.append(f"    trainerbattle_double({trainer_id}, {script_name}_Text_Intro, {script_name}_Text_Defeat, {script_name}_Text_NotEnoughMons)")
                lines.append(f"    msgbox({script_name}_Text_PostBattle, MSGBOX_AUTOCLOSE)")
                lines.append("    end")
                lines.append("}")
                lines.append("")
                ...
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_NotEnoughMons {{")
                lines.append('    "You need at least two Pokemon."')
                lines.append("}")
            elif b_type == "Multi (1v2)":
                lines.append(f"    trainerbattle_two_trainers({trainer_id}, {script_name}_Text_DefeatA, TRAINER_B, {script_name}_Text_DefeatB)")
                lines.append(f"    msgbox({script_name}_Text_PostBattle, MSGBOX_AUTOCLOSE)")
                lines.append("    end")
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_DefeatA {{")
                lines.append('    "I lost..."')
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_DefeatB {{")
                lines.append('    "Me too..."')
                lines.append("}")
                lines.append("")
                lines.append(f"text {script_name}_Text_PostBattle {{")
                lines.append('    "Good battle."')
                lines.append("}")
            elif b_type == "Multi (2v2)":
                lines.append(f"    setmultitrainerbattle({trainer_id}, {script_name}_Text_DefeatA, TRAINER_B, {script_name}_Text_DefeatB, PARTNER_NONE)")
                lines.append(f"    # Replace DoMultiTrainerBattle if a custom setup is needed")
                lines.append(f"    special(DoMultiTrainerBattle)")
                lines.append(f"    waitstate")
                lines.append(f"    msgbox({script_name}_Text_PostBattle, MSGBOX_AUTOCLOSE)")
                lines.append("    end")
                lines.append("}")
                lines.append("")
        ...
        battle_type_var.trace_add("write", update_script)
        update_script()
        
        def copy_to_clipboard() -> None:
            dialog.clipboard_clear()
            dialog.clipboard_append(text_widget.get("1.0", tk.END))
            messagebox.showinfo("Copied", "Script copied to clipboard!", parent=dialog)
            
        ttk.Button(top_frame, text="Copy to Clipboard", command=copy_to_clipboard).pack(side=tk.RIGHT)
    ...
    def move_pokemon_up(self) -> None:
        if self.current is None or self.current_mon_index is None or self.current_mon_index <= 0:
            return
        
        idx = self.current_mon_index
        self.current.pokemon[idx - 1], self.current.pokemon[idx] = self.current.pokemon[idx], self.current.pokemon[idx - 1]
        self.refresh_party()
        self.party_list.selection_set(idx - 1)
        self.on_mon_select()

    def move_pokemon_down(self) -> None:
        if self.current is None or self.current_mon_index is None or self.current_mon_index >= len(self.current.pokemon) - 1:
            return
    ...
    def import_showdown(self) -> None:
        ...
        def do_import() -> None:
            ...
            for chunk in chunks:
                ...
                if mon is not None:
                    self.current.pokemon.append(mon)
            
            self.refresh_party()
            dialog.destroy()
            if self.current.pokemon:
                last = len(self.current.pokemon) - 1
                self.party_list.selection_clear(0, tk.END)
                self.party_list.selection_set(last)
                self.on_mon_select()

        ttk.Button(dialog, text="Import", command=do_import).pack(side=tk.BOTTOM, pady=8)
    ...
    def update_current_mon_from_form(self) -> None:
        ...
        title = re.sub(r"\s*\([MF]\)\s*", "", title).strip()
        
        gender = self.mon_vars["Gender"].get().strip() if "Gender" in self.mon_vars else ""
        if gender == "Male":
            title += " (M)"
        elif gender == "Female":
            title += " (F)"
        ...
        for key, var in self.mon_vars.items():
            value = var.get().strip()
            if key == "Dynamax Level":
                if value == "Yes":
                    value = "10"
                else:
                    value = ""
            elif key in {"Gigantamax", "Shiny"} and value == "No":
                value = ""
    ...
    def load_photo_frames(self, path: Path, max_size: int, crop_pokemon_frame: bool = False, animate: bool = False) -> list[tk.PhotoImage]:
        if Image is not None and ImageTk is not None:
            try:
                ...
                frames = []
                if (crop_pokemon_frame or animate) and height > width and height % width == 0:
                    num_frames = height // width if animate else 1
                    for i in range(num_frames):
                        frame = image.crop((0, i * width, width, (i + 1) * width))
                        scale = max(1, min(3, max_size // max(width, width, 1)))
                        if scale > 1:
                            nearest = getattr(getattr(Image, "Resampling", Image), "NEAREST")
                            frame = frame.resize((width * scale, width * scale), nearest)
                        frames.append(ImageTk.PhotoImage(frame))
                elif (crop_pokemon_frame or animate) and width > height and width % height == 0:
                    num_frames = width // height if animate else 1
                    for i in range(num_frames):
                        frame = image.crop((i * height, 0, (i + 1) * height, height))
                        scale = max(1, min(3, max_size // max(height, height, 1)))
                        if scale > 1:
                            nearest = getattr(getattr(Image, "Resampling", Image), "NEAREST")
                            frame = frame.resize((height * scale, height * scale), nearest)
                        frames.append(ImageTk.PhotoImage(frame))
                else:
    ...
    def load_photo(self, path: Path, max_size: int, crop_pokemon_frame: bool = False) -> tk.PhotoImage | None:
        frames = self.load_photo_frames(path, max_size, crop_pokemon_frame=crop_pokemon_frame, animate=False)
    ...
    def update_mon_preview(self, mon: PokemonEntry) -> None:
        ...
        self.animated_labels = [item for item in self.animated_labels if item[0] != self.mon_preview]
        if len(frames) > 1:
            self.animated_labels.append((self.mon_preview, frames))
    ...
    def render_party_previews(self) -> None:
        ...
        for index, mon in enumerate(self.current.pokemon[:6]):
            path = pokemon_icon_path(mon.species)
            ...
            if path is not None:
                frames = self.load_photo_frames(path, 32, crop_pokemon_frame=True, animate=True)
                if frames:
                    self.mon_images.extend(frames)
                    label = ttk.Label(frame, image=frames[0], anchor=tk.CENTER)
                    label.pack()
