...
#define B_FLAG_TERA_ORB_CHARGED     FLAG_TERA_ORB_CHARGE     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
#define B_FLAG_TERA_ORB_NO_COST     FLAG_UNUSED_0x91D     // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In S/V, this occurs after an event with Terapagos.
...
#define B_SHOW_TYPES        SHOW_EFFECTIVENESS_ALWAYS     // When to show type indicators next to Pokémon health bars in battle, while choosing a move after selecting a target Pokémon.
