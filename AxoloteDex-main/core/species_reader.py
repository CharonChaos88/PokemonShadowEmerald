...
class SpeciesReader:
    ...
    def read(self, root: Path) -> SpeciesReadResult:
        ...
        if project.teachable_file.exists():
            ...
            teachable_map = {}
            self.warn(
                "There is no teachable_learnsets.h; continues without teachable map",
    ...
    def _validate_project(self, root: Path) -> ExpansionProject:
        if not root.exists() or not root.is_dir():
            raise FileNotFoundError(f"Invalid route: {root}")
        ...
        if not family_files:
            raise FileNotFoundError("were not found gen_*_families.h")
        if not level_up_files:
            raise FileNotFoundError("They were not found level_up_learnsets/gen_*.h")
    ...
    def _parse_species_constants(self, species_header: Path) -> list[PokemonSpecies]:
        ...
        if not out:
            raise ValueError("Constants could not be extracted SPECIES_*")
        return out
        ...
        for family_file in family_files:
            ...
            for match in species_start.finditer(text):
                ...
                if constant not in species_map:
                    self.warn(
                        "Species info block without constant in species.h",
                        family_file,
    ...
    def _parse_graphics_symbols(self, graphics_file: Path) -> dict[str, str]:
        ...
        if not symbol_map:
            self.warn("Could not extract graphic symbols", graphics_file)
        return symbol_map
    ...
    def _parse_level_up_files(self, files: list[Path]) -> tuple[dict[str, list[str]], dict[str, list[dict[str, int | str]]]]:
        ...
        for file_path in files:
            ...
            for m in table_re.finditer(text):
                ...
                if end < 0:
                    self.warn("Could not close table learnset", file_path, context=symbol)
                    continue
        ...
        for s in species_list:
            ...
            if level_symbol:
                ...
                if level_symbol not in level_up_map:
                    self.warn("No level up move learnset", context=f"{s.constant_name} -> {level_symbol}")
            if egg_symbol:
                s.egg_moves_raw = egg_map.get(egg_symbol, [])
                if egg_symbol not in egg_map:
                    self.warn("No egg move learnset", context=f"{s.constant_name} -> {egg_symbol}")
            if teach_symbol:
                s.teachable_moves_raw = teachable_map.get(teach_symbol, [])
                if teach_symbol not in teachable_map:
                    self.warn("No tutor move learnset", context=f"{s.constant_name} -> {teach_symbol}")
