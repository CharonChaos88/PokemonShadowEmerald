...
def lint_species_definition(
    ...
    if mode == "add" and const in existing_constants:
        result.errors.append(f"constant duplicate: {const}")
    if mode == "edit" and const not in existing_constants:
        result.errors.append(f"constant does not exist for edit: {const}")
    if mode == "delete" and const not in existing_constants:
        result.errors.append(f"constant does not exist for delete: {const}")
    if mode == "delete":
        return result
    if not name:
        result.errors.append("empty name")
    elif len(name) > 12:
    ...
    for key in ["hp", "attack", "defense", "speed", "sp_attack", "sp_defense"]:
        ...
        if not isinstance(val, int) or val < 1 or val > 1000:
            result.errors.append(f"stats out of range: {key}={val}")

    cry_id = str(data.get("cry_id", "")).strip()
    if cry_id and not cry_id.startswith("CRY_"):
        result.errors.append(f"cry_id invalid: {cry_id}")
    ...
    if isinstance(ev_yields, dict):
        for key in ["hp", "attack", "defense", "speed", "sp_attack", "sp_defense"]:
            ...
            if not isinstance(val, int) or val < 0 or val > 3:
                result.errors.append(f"ev_yields out of range: {key}={val}")
    else:
        result.errors.append("ev_yields invalid: object expected")
    ...
    if len(abilities) < 3:
        ...
        result.warnings.append("autogenerated missing hidden ability a ABILITY_NONE")
    elif not abilities[2]:
        abilities[2] = "ABILITY_NONE"
        result.normalized_data["abilities"] = abilities
        result.warnings.append("hidden ability autogenerated void a ABILITY_NONE")
    ...
    if not isinstance(learnset, list) or len(learnset) == 0:
        ...
        result.warnings.append(f"without learnset: basic autogenerated with {basic_move}")
    else:
        for i, row in enumerate(learnset):
            if not isinstance(row, dict):
                result.errors.append(f"level_up_learnset invalid row in index {i}")
                continue
            level = row.get("level")
            move = str(row.get("move", "")).strip()
            if not isinstance(level, int) or level < 1 or level > 100:
                result.errors.append(f"level_up_learnset invalid level in the index {i}: {level}")
            if not move.startswith("MOVE_"):
                result.errors.append(f"level_up_learnset invalid move in index {i}: {move}")
    ...
    if isinstance(tmhm, list):
        for i, move in enumerate(tmhm):
            ...
            if not m.startswith("MOVE_"):
                result.errors.append(f"tmhm_learnset invalid move in index {i}: {m}")
    ...
    if isinstance(tutor, list):
        for i, move in enumerate(tutor):
            m = str(move).strip()
            if not m.startswith("MOVE_"):
                result.errors.append(f"tutor_learnset invalid move in index {i}: {m}")

    if using_fallback_assets:
        result.warnings.append("using fallback (Bulbasaur)")
