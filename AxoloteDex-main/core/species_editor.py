...
class SpeciesEditor:
    ...
    def build_plan(self, data: dict, validation: ValidationResult) -> ChangePlan:
        ...
        if mode not in {"add", "edit", "delete"}:
            plan.errors.append("mode not supported")
        ...
        plan.strategy_notes.append(
            "New constant inserted just before SPECIES_EGG and SPECIES_EGG is updated to point to the new species."
        )
        plan.strategy_notes.append(
            f"Kind block is inserted at the end of {relpath(family_file, self.project_root)} before the block __INTELLISENSE__."
        )
        plan.strategy_notes.append(
        ...
        plan.add_step(
            ChangeStep(
                ...
                action="update",
                reason="Add species constant and recalculate SPECIES_EGG",
        ...
        plan.add_step(
            ChangeStep(
                ...
                action="insert",
                reason="Add graphic species symbols",
        ...
        if target_asset_dir.exists():
            plan.warnings.append(
                f"Existing folder detected: graphics/pokemon/{data['folder_name']}. Reuse mode is used (references only), without copying assets."
            )
        else:
            for asset_name, source in validation.asset_sources.items():
                ...
                plan.add_step(
                    ChangeStep(
                        ...
                        action="copy_asset",
                        reason=f"Copy required asset ({asset_name})",
                        new_text=str(source),
                        risk_level="low",
                        warnings=[
                            "Asset copied without conversion; The conversion is handled by the project build."
                        ],
                    )
                )
    ...
    def _build_edit_plan(self, plan: ChangePlan, data: dict) -> ChangePlan:
        ...
        if species is None:
            plan.errors.append(f"The species does not exist {constant_name}")
            return plan
            ...
            if new_block != old_block:
                plan.add_step(
                    ChangeStep(
                        target_file=relpath(family_file, self.project_root),
                        action="update",
                        reason="Update evolution of the species",
                        old_text=old_block,
                        new_text=new_block,
                        risk_level="high",
                    )
        ...
        if not family_updated:
            plan.warnings.append(f"No species info block found for {constant_name}")
        ...
        if new_folder and new_folder != current_folder:
            ...
            if old_graphics_block:
                ...
                if old_graphics_block != new_graphics_block:
                    plan.add_step(
                        ChangeStep(
                            target_file=relpath(graphics_h, self.project_root),
                            action="update",
                            reason="Update graphic folder referenced by the species",
                ...
                if folder_path.exists():
                    plan.warnings.append(
                        f"Existing folder detected: graphics/pokemon/{new_folder}. Active reuse mode (no asset copies)."
                    )
                else:
                    plan.warnings.append(
                        f"The folder graphics/pokemon/{new_folder} does not exist. Check assets before compiling."
                    )

        level_symbol = species.level_up_symbol
        if not level_symbol:
            plan.errors.append(f"levelUpLearnset was not detected for {constant_name}")
            return plan

        level_file, old_block = self._find_levelup_block(level_symbol)
        if level_file is None or old_block is None:
            plan.errors.append(f"No block found {level_symbol}")
            return plan
        ...
        plan.add_step(
            ChangeStep(
                ...
                action="update",
                reason="Update level-up learnset",
                old_text=old_block,
                new_text=new_block,
                risk_level="high",
            )
        ...
        if new_value != old_value:
            ...
            plan.add_step(
                ChangeStep(
                    ...
                    action="update",
                    reason="Update source of learnables (TM/HM + Tutor)",
                    old_text=old_json,
                    new_text=new_json,
                    risk_level="high",
                )
            ...
            plan.add_step(
                ChangeStep(
                    ...
                    action="run_script",
                    reason="Regenerate teachable_learnsets.h from all_learnables.json",
    ...
    def _build_delete_plan(self, plan: ChangePlan, data: dict, species_h: Path, graphics_h: Path) -> ChangePlan:
        ...
        if external_refs:
            ...
            if should_replace:
                ...
                if residuals:
                    plan.errors.append("replace+delete failed to clear all references to the species")
                    for ref in residuals[:12]:
                        plan.errors.append(f"ref residual: {ref}")
                    if len(residuals) > 12:
                        plan.errors.append(f"... y {len(residuals) - 12} residual references more")
                    return plan
                ...
                if len(replacements_summary) > 20:
                    plan.warnings.append(f"... y {len(replacements_summary) - 20} replacements more")
            elif delete_mode == "force-delete":
                plan.warnings.append("force-delete active: will be deleted even if external references exist")
                ...
                plan.errors.append(
                    "Cannot delete species: external references exist outside of species_info/graphics/species.h"
                )
                for ref in external_refs[:12]:
                    plan.errors.append(f"ref: {ref}")
                if len(external_refs) > 12:
                    plan.errors.append(f"... y {len(external_refs) - 12} more references")
                plan.warnings.append("Tip: use delete_mode='replace+delete' (Advanced) to automatically replace references")
                return plan
        ...
        if not constant_match:
            plan.errors.append(f"Could not locate define {constant_name} en species.h")
            return plan
        ...
        plan.add_step(
            ChangeStep(
                target_file=relpath(species_h, self.project_root),
                action="update",
                reason="Delete species constant and recalculate SPECIES_EGG",
                old_text=old_species_h,
                new_text=new_species_h,
                risk_level="high",
            )
            ...
            if block:
                plan.add_step(
                    ChangeStep(
                        target_file=relpath(family_file, self.project_root),
                        action="update",
                        reason="Delete species info block",
                        old_text=block,
                        new_text="",
                        risk_level="high",
                    )
        ...
        if graphics_block:
            plan.add_step(
                ChangeStep(
                    target_file=relpath(graphics_h, self.project_root),
                    action="update",
                    reason="Remove graphic species symbols",
            ...
            if graphics_lines:
                for line in graphics_lines:
                    plan.add_step(
                        ChangeStep(
                            target_file=relpath(graphics_h, self.project_root),
                            action="update",
                            reason="Remove residual graphic symbol from species",
                            old_text=line + "\n",
                            new_text="",
                            risk_level="medium",
                        )
                ...
                plan.warnings.append(f"No graphic block found for {constant_name}")
        ...
        if folder_name:
            if self._folder_is_shared_with_other_species(constant_name, folder_name):
                plan.warnings.append(
                    f"It is not removed graphics/pokemon/{folder_name}: is shared by other species"
                )
            else:
                ...
                plan.add_step(
                    ChangeStep(
                        ...
                        action="delete_dir",
                        reason="Delete species assets folder",
                        new_text="",
                        risk_level="high",
                    )
                )
        else:
            plan.warnings.append("folder_name not detected for deleting assets")

        return plan

    def apply_plan(self, plan: ChangePlan, validation: ValidationResult) -> ApplyResult:
        if plan.is_blocked:
            return ApplyResult(applied=False, backup_dir=None, messages=["Plan locked; does not apply"]) 
