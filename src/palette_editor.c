...
const u16* GetUnifiedMonPalette(u16 species, bool32 isShiny, u32 personality, u8 altPalette) {
    ...
    if (altPalette == 0 || species >= NUM_SPECIES) return defaultPal;
...
static void CB2_PaletteEditorRunner(void) { RunTasks(); BuildOamBuffer(); UpdatePaletteFade(); }
...
static void PrintEditorUI(struct PaletteEditor *data) {
    ...
    // Slot
    if (data->editChannel == 0) StringCopy(gStringVar4, sText_ArrowSlot); else StringCopy(gStringVar4, sText_EmptySlot);
    ...
    // Index
    if (data->editChannel == 1) StringCopy(gStringVar4, sText_ArrowIndex); else StringCopy(gStringVar4, sText_EmptyIndex);
    ...
    // Red
    if (data->editChannel == 2) StringCopy(gStringVar4, sText_ArrowRed); else StringCopy(gStringVar4, sText_EmptyRed);
    ...
    // Green
    if (data->editChannel == 3) StringCopy(gStringVar4, sText_ArrowGreen); else StringCopy(gStringVar4, sText_EmptyGreen);
    ...
    // Blue
    if (data->editChannel == 4) StringCopy(gStringVar4, sText_ArrowBlue); else StringCopy(gStringVar4, sText_EmptyBlue);
...
void CB2_InitPaletteEditor(void) {
    u8 taskId; struct PaletteEditor *data; u16 species; u32 personality; const u16 *pal;
    switch (gMain.state) {
    case 0: SetVBlankCallback(NULL); FreeMonSpritesGfx(); SetGpuReg(REG_OFFSET_DISPCNT, 0); DmaFillLarge16(3, 0, (u8 *)VRAM, VRAM_SIZE, 0x1000); DmaClear32(3, OAM, OAM_SIZE); DmaClear16(3, PLTT, PLTT_SIZE); gMain.state++; break;
    case 1: ResetTasks(); ResetSpriteData(); ResetPaletteFade(); FreeAllSpritePalettes(); gReservedSpritePaletteCount = 8; ResetAllPicSprites(); BlendPalettes(PALETTES_ALL, 16, RGB_BLACK); gMain.state++; break;
    case 2: InitBgsFromTemplates(0, (struct BgTemplate[]){{0,0,24,0,0,0,0},{1,1,28,0,0,0,0}}, 2); FreeAllWindowBuffers(); InitWindows((struct WindowTemplate[]){{0,14,2,15,16,15,1}, DUMMY_WIN_TEMPLATE}); LoadPalette(GetTextWindowPalette(0), BG_PLTT_ID(15), 32); LoadStdWindowGfx(0, 0x214, 14); gMain.state++; break;
    case 3: AllocateMonSpritesGfx(); SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP | DISPCNT_BG0_ON); ShowBg(0);
        taskId = CreateTask(HandleInput_PaletteEditor, 0); data = AllocZeroed(sizeof(struct PaletteEditor)); 
        *(struct PaletteEditor**)(&gTasks[taskId].data[0]) = data;
        
        if (gSpecialVar_0x8005) BoxMonToMon(&gPokemonStoragePtr->boxes[gSpecialVar_MonBoxId][gSpecialVar_MonBoxPos], &data->mon); 
        else data->mon = gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004];
        
        data->currentAlt = GetMonData(&data->mon, MON_DATA_ALT_PALETTE); 
        if (data->currentAlt == 0) data->currentAlt = 1;
        ...
        LoadSpriteSheet(&frontSheet); LoadSpriteSheet(&backSheet);
        
        SetMultiuseSpriteTemplateToPokemon(species, 1); gMultiuseSpriteTemplate.paletteTag = personality; gMultiuseSpriteTemplate.tileTag = 0x1234;
        ...
        gSprites[data->frontSpriteId].callback = SpriteCallbackDummy; gSprites[data->frontSpriteId].animPaused = TRUE;

        SetMultiuseSpriteTemplateToPokemon(species, 2); gMultiuseSpriteTemplate.paletteTag = personality; gMultiuseSpriteTemplate.tileTag = 0x1235;
        ...
        gSprites[data->backSpriteId].callback = SpriteCallbackDummy; gSprites[data->backSpriteId].animPaused = TRUE;
        
        PrintEditorUI(data); gMain.state++; break;
    case 4: EnableInterrupts(1); SetVBlankCallback(VBlankCB); SetMainCallback2(CB2_PaletteEditorRunner); BeginNormalPaletteFade(PALETTES_ALL, 0, 16, 0, RGB_BLACK); break;
    }
}
...
static void HandleInput_PaletteEditor(u8 taskId) {
    ...
    if (JOY_NEW(A_BUTTON) || JOY_NEW(B_BUTTON)) {
        ...
        if (gSpecialVar_0x8005) SetBoxMonData(&gPokemonStoragePtr->boxes[gSpecialVar_MonBoxId][gSpecialVar_MonBoxPos], MON_DATA_ALT_PALETTE, &data->currentAlt); 
        else SetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_ALT_PALETTE, &data->currentAlt);
        
        PlaySE(SE_PC_OFF); BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK); gTasks[taskId].func = Exit_PaletteEditor; return;
    }

    // Move Cursor Up/Down
    if (JOY_NEW(DPAD_UP)) { 
        if (data->editChannel == 0) data->editChannel = 4; else data->editChannel--; 
        PrintEditorUI(data); 
        PlaySE(SE_SELECT); 
    }
    ...
    if (JOY_REPEAT(DPAD_LEFT) || JOY_REPEAT(DPAD_RIGHT)) {
        ...
        case 0: data->currentAlt = (data->currentAlt + dir + 3) % 3; memcpy(data->mutablePalette, GetUnifiedMonPalette(species, data->isShiny, personality, data->currentAlt), 32); changed = TRUE; break;
        case 1: data->editColorIndex = (data->editColorIndex + dir - 1 + 15) % 15 + 1; changed = TRUE; break;
        case 2: if (data->currentAlt > 0 && (int)r + dir >= 0 && (int)r + dir <= 31) { r += dir; changed = TRUE; } break;
        case 3: if (data->currentAlt > 0 && (int)g + dir >= 0 && (int)g + dir <= 31) { g += dir; changed = TRUE; } break;
        case 4: if (data->currentAlt > 0 && (int)b + dir >= 0 && (int)b + dir <= 31) { b += dir; changed = TRUE; } break;
        }

        if (changed) {
            if (data->editChannel >= 2) *colorPtr = RGB(r, g, b);
            ...
            PrintEditorUI(data); PlaySE(SE_SELECT);
        }
    }
}
...
static void Exit_PaletteEditor(u8 taskId) {
    if (!gPaletteFade.active) {
        struct PaletteEditor *data = *(struct PaletteEditor**)(&gTasks[taskId].data[0]);
        Free(data); FreeMonSpritesGfx(); DestroyTask(taskId);
